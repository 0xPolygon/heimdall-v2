...
replace (
	...
	// TODO marcello: replace with the new tag (e.g. v0.2.9-polygon) once the matching
	// 0xPolygon/cosmos-sdk PR (branch mardizzone/expedite_proposals_fix → devel) is merged and tagged.
	// Local path used here only so that `go mod tidy` and tests can run locally before the dep is published.
