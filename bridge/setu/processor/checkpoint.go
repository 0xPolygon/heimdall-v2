...
func (cp *CheckpointProcessor) sendCheckpointToRootchain(eventBytes string, blockHeight int64) error {
	...
	var (
		startBlock uint64
		endBlock   uint64
		// TODO HV2 - uncomment when we uncomment the below code snippet
		// txHash     string
	)
	...
	if shouldSend && isCurrentProposer {
		// TODO HV2 - uncomment when we uncomment createAndSendCheckpointToRootchain()
		/*
...
func (cp *CheckpointProcessor) sendCheckpointAckToHeimdall(eventName string, checkpointAckStr string) error {
	...
	if err = helper.UnpackLog(cp.rootchainAbi, event, eventName, &log); err != nil {
		...
		msg := checkpointtypes.NewMsgCheckpointAck(
			...
			event.Start.Uint64(),
			event.End.Uint64(),
			hmTypes.HeimdallHash{Hash: event.Root[:]},
			hmTypes.HeimdallHash{Hash: log.TxHash.Bytes()},
			uint64(log.Index),
		)
...
func (cp *CheckpointProcessor) nextExpectedCheckpoint(checkpointContext *CheckpointContext, latestChildBlock uint64) (*ContractCheckpoint, error) {
	...
	_currentHeaderBlock, err := cp.contractCaller.CurrentHeaderBlock(rootChainInstance, checkpointParams.ChildBlockInterval)
	if err != nil {
	...
	// get header info
	_, currentStart, currentEnd, lastCheckpointTime, _, err := cp.contractCaller.GetHeaderInfo(currentHeaderBlockNumber.Uint64(), rootChainInstance, checkpointParams.ChildBlockInterval)
	if err != nil {
...
func (cp *CheckpointProcessor) createAndSendCheckpointToHeimdall(checkpointContext *CheckpointContext, start uint64, end uint64) error {
	...
	msg := checkpointtypes.NewMsgCheckpointBlock(
		...
		hmTypes.HeimdallHash{Hash: root},
		accountRootHash,
...
func (cp *CheckpointProcessor) getLatestCheckpointTime(checkpointContext *CheckpointContext) (int64, error) {
	...
	lastHeaderNumber, err := cp.contractCaller.CurrentHeaderBlock(rootChainInstance, checkpointParams.ChildBlockInterval)
	if err != nil {
	...
	// header block
	_, _, _, createdAt, _, err := cp.contractCaller.GetHeaderInfo(lastHeaderNumber, rootChainInstance, checkpointParams.ChildBlockInterval)
	if err != nil {
