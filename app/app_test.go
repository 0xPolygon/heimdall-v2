...
func TestEndBlockerEmitsTransferEvent(t *testing.T) {
    ...
    for _, event := range result.Events {
        if event.Type == heimdalltypes.EventTypeFeeTransfer {
    ...
    for _, attr := range feeTransferEvent.Attributes {
        switch attr.Key {
        case heimdalltypes.AttributeKeyProposer:
            proposerAttr = &attr
        case heimdalltypes.AttributeKeyDenom:
            denomAttr = &attr
        case heimdalltypes.AttributeKeyAmount:
            amountAttr = &attr
        }
    }
