...
func TestFullAppSimulation(t *testing.T) {
	...
	db, dir, logger, skip, err := simtestutil.SetupSimulation(config, "leveldb-app-heimdall", "Simulation", simcli.FlagVerboseValue, simcli.FlagEnabledValue)
	if skip {
	...
	app := NewHeimdallApp(logger, db, nil, true, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(HeimdallAppChainID))
	require.Equal(t, "HeimdallApp", app.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	// TODO CHECK HEIMDALL-V2: We should add the relevant modules when implemented in the modulesToExport list
	exported, err := app.ExportAppStateAndValidators(false, []string{}, []string{})
	require.NoError(t, err)

	fmt.Printf("importing genesis...\n")

	newDB, newDir, _, _, err := simtestutil.SetupSimulation(config, "leveldb-app-heimdall-2", "Simulation-2", simcli.FlagVerboseValue, simcli.FlagEnabledValue)
	require.NoError(t, err, "simulation setup failed")
