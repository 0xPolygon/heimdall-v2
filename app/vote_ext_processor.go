...
type (
	// ModVoteExtHandler is executed in the respective modules while extending vote
	ModVoteExtHandler func(ctx sdk.Context, msg sdk.Msg) (types.Vote, error)

	// ModPostHandler is executed in the respective modules to persist vote extension data once validated by 2/3+ majority
	ModPostHandler func(ctx sdk.Context, msg sdk.Msg, res types.Vote) error
)

// VoteExtensionHandler handles Vote Extension processing for Heimdall app
type VoteExtensionProcessor struct {
	app               *HeimdallApp
	modVoteExtHandler map[string]ModVoteExtHandler
	modPostHandler    map[string]ModPostHandler
}
...
func (v *VoteExtensionProcessor) ExtendVote() sdk.ExtendVoteHandler {
	return func(ctx sdk.Context, req *abci.RequestExtendVote) (*abci.ResponseExtendVote, error) {
		...
		if len(req.Txs) > 1 || (len(req.Txs) >= 1 && req.Height == ctx.ConsensusParams().Abci.VoteExtensionsEnableHeight) {
			...
			for _, rawTx := range txs {
				...
				for _, msg := range msgs {
					fn, ok := v.modVoteExtHandler[sdk.MsgTypeURL(msg)]
					if !ok {
						continue
					}
					...
					res, err := fn(ctx, msg)
					if err != nil {
						logger.Error("Error occurred while extending vote", "msg", msg, "error", err)
						continue
					}
		...
		canonicalSideTxRes := types.CanonicalSideTxResponse{
...
func (h *VoteExtensionProcessor) VerifyVoteExtension() sdk.VerifyVoteExtensionHandler {
	return func(ctx sdk.Context, req *abci.RequestVerifyVoteExtension) (*abci.ResponseVerifyVoteExtension, error) {
		...
		for _, v := range ve.SideTxResponses {
			...
			isValidVote := v.Result == types.Vote_VOTE_YES || v.Result == types.Vote_VOTE_NO || v.Result == types.Vote_VOTE_SKIP
			if !isValidVote {
