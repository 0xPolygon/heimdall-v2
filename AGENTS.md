...
Treat "is there a Bor reachable from this Heimdall?" as an **open
question, not an invariant**. Any feature that introduces a hard
Heimdall→Bor dependency will break Heimdall-only operators and M:N
fan-outs where the assumed Bor isn't there. Design new features so they
degrade gracefully (fail-open / optional peer / config-driven) when Bor
isn't reachable.
