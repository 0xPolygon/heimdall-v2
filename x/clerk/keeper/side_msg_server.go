...
func (srv *sideMsgServer) SideHandleMsgEventRecord(ctx sdk.Context, _msg sdk.Msg) (result sidetxs.Vote) {
	...
	if !ok {
		...
		srv.Logger(ctx).Info("Cosmoverse25 - StateSync - side_msg_server - sideHandler: failed, voting NO", "height", ctx.BlockHeight())
	...
	srv.Logger(ctx).Info("Cosmoverse25 - StateSync - side_msg_server - sideHandler: SUCCESS, voting YES", "height", ctx.BlockHeight())
...
func (srv *sideMsgServer) PostHandleMsgEventRecord(ctx sdk.Context, _msg sdk.Msg, sideTxResult sidetxs.Vote) error {
	...
	if !ok {
		...
		srv.Logger(ctx).Info("Cosmoverse25 - StateSync - side_msg_server - postHandler: failed, not updating state to send stateSync to bor", "height", ctx.BlockHeight())
	}
	...
	srv.Logger(ctx).Info("Cosmoverse25 - StateSync - side_msg_server - postHandler: SUCCESS, updating the state to send stateSync to bor", "height", ctx.BlockHeight())
