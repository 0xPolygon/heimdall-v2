...
func (s *KeeperTestSuite) sideHandler(ctx sdk.Context, msg sdk.Msg) mod.Vote {
	cfg := s.sideMsgCfg
	return cfg.GetSideHandler(msg)(ctx, msg)
}

func (s *KeeperTestSuite) postHandler(ctx sdk.Context, msg sdk.Msg, vote mod.Vote) {
	cfg := s.sideMsgCfg
...
func (s *KeeperTestSuite) TestPostHandleTopupTx() {
	...
	t.Run("no result", func(t *testing.T) {
		...
		postHandler(ctx, &msg, mod.Vote_VOTE_NO)
		ok, err := keeper.HasTopupSequence(ctx, sequence.String())
		require.NoError(err)
		require.False(ok)
	})
