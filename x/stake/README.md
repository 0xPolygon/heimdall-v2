...
The `stake` module in Heimdall is responsible for a validator's stake related operations. It primarily aids in
...
currentValidatorSet := app.StakeKeeper.GetValidatorSet(ctx)
...
```
heimdallcli tx stake validator-join --proposer <PROPOSER_ADDRESS> --tx-hash <ETH_TX_HASH> --signer-pubkey <PUB_KEY> --staked-amount <STAKED_AMOUNT> --activation-epoch <ACTIVATION_EPOCH>
```

Or the REST server :

```
curl -X POST "localhost:1317/stake/validator-join?from=<PROPOSER_ADDRESS>&tx-hash=<ETH_TX_HASH>&signer-pubkey=<PUB_KEY>&staked-amount=<STAKED_AMOUNT>&activation-epoch=<ACTIVATION_EPOCH>"
```
...
```
heimdallcli tx stake validator-exit --proposer <PROPOSER_ADDRESS> --id <VALIDATOR_ID> --tx-hash <ETH_TX_HASH> --nonce <VALIDATOR_NONCE> --log-index <LOG_INDEX> --block-number <BLOCK_NUMBER> --deactivation-epoch <DEACTIVATION_EPOCH>
```

Or the REST server :

```
curl -X POST "localhost:1317/stake/validator-exit?from=<PROPOSER_ADDRESS>&id=<VALIDATOR_ID>&tx-hash=<ETH_TX_HASH>&nonce=<VALIDATOR_NONCE>&log-index=<LOG_INDEX>&block-number=<BLOCK_NUMBER>&deactivation-epoch=<DEACTIVATION_EPOCH>"
```
...
```
heimdallcli tx stake stake-update --proposer <PROPOSER_ADDRESS> --id <VALIDATOR_ID> --tx-hash <ETH_TX_HASH> --staked-amount <STAKED_AMOUNT> --nonce <VALIDATOR_NONCE> --log-index <LOG_INDEX> --block-number <BLOCK_NUMBER>
```

Or the REST server :

```
curl -X POST "localhost:1317/stake/stake-update?proposer=<PROPOSER_ADDRESS>&id=<VALIDATOR_ID>&tx-hash=<ETH_TX_HASH>&staked-amount=<STAKED_AMOUNT>&nonce=<VALIDATOR_NONCE>&log-index=<LOG_INDEX>&block-number=<BLOCK_NUMBER>"
```
...
```
heimdallcli tx stake signer-update --proposer <PROPOSER_ADDRESS> --id <VALIDATOR_ID> --new-pubkey <NEW_PUBKEY> --tx-hash <ETH_TX_HASH> --nonce <VALIDATOR_NONCE> --log-index <LOG_INDEX> --block-number <BLOCK_NUMBER>
```

Or the REST server : 

```
curl -X POST "localhost:1317/stake/signer-update?proposer=<PROPOSER_ADDRESS>&id=<VALIDATOR_ID>&new-pubkey=<NEW_PUBKEY>&tx-hash=<ETH_TX_HASH>&nonce=<VALIDATOR_NONCE>&log-index=<LOG_INDEX>&block-number=<BLOCK_NUMBER>"
```
...
```
heimdallcli query stake validator-info --id=<VALIDATOR_ID>

OR

heimdallcli query stake validator-info --validator=<VALIDATOR_ADDRESS>
```

```
heimdallcli query stake current-validator-set
```

```
heimdallcli query stake staking-power
```

```
heimdallcli query stake validator-status --validator=<VALIDATOR_ADDRESS>
```

```
heimdallcli query stake proposer --times=<TIMES>
```

```
heimdallcli query stake current-proposer 
```

```
heimdallcli query stake is-old-tx --tx-hash=<ETH_TX_HASH> --log-index=<LOG_INDEX>
```
...
```
curl localhost:1317/stake/validator/<VALIDATOR_ID>

OR

curl localhost:1317/stake/validator/<VALIDATOR_ADDRESS>
```
...
```
curl localhost:1317/stake/totalpower
```

```
curl localhost:1317/stake/validator-status/<VALIDATOR_ADDRESS>
```

```
curl localhost:1317/stake/proposer/<TIMES>
```

```
curl "localhost:1317/stake/current-proposer
```

```
curl localhost:1317/stake/isoldtx?tx-hash=<ETH_TX_HASH>&log-index=<LOG_INDEX>
```
