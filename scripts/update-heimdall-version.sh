...
# TODO-HV2: test this script once app versioning files (e.g. packaging templates) are ready
...
# VersionMajor
read -r -p "* VersionMajor: " VersionMajor
if [ -z "$VersionMajor" ]
then
...
# VersionMinor
read -r -p "* VersionMinor: " VersionMinor
if [ -z "$VersionMinor" ]
then
...
# VersionPatch
read -r -p "* VersionPatch: " VersionPatch
if [ -z "$VersionPatch" ]
then
...
read -r -p "* VersionMeta (optional, press enter if not needed): " VersionMeta
if [[ -n "$VersionMeta" ]]
then
    version+="-"$VersionMeta
fi
