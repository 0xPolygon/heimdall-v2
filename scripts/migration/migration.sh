...
show_help() {
  echo "Usage: sudo ./migrate.sh --heimdall-home=PATH --cli-path=PATH --d-path=PATH \\
            --network=mainnet|amoy --nodetype=sentry|validator \\
            --backup-dir=PATH --moniker=MONIKER --service-user=USER \\
            --generate-genesis=true|false [--bor-path=PATH]"
...
if ! "$HEIMDALL_CLI_PATH" version &> /dev/null; then
    handle_error $STEP "HEIMDALL_CLI_PATH is invalid or heimdallcli is not executable."
fi
RAW_HEIMDALLCLI_VERSION=$($HEIMDALL_CLI_PATH version)
CLEAN_REQUIRED_VERSION=${REQUIRED_HEIMDALLCLI_VERSION#v}
CLEAN_FOUND_VERSION=$(echo "$RAW_HEIMDALLCLI_VERSION" | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -n1 | sed 's/^v//')
if ! version_ge "$CLEAN_FOUND_VERSION" "$CLEAN_REQUIRED_VERSION"; then
    handle_error $STEP "heimdallcli version must be >= $REQUIRED_HEIMDALLCLI_VERSION. Found: $RAW_HEIMDALLCLI_VERSION"
fi
# HEIMDALLD_PATH
...
# Step 4: Generate or download Heimdall v1 genesis JSON
STEP=4
...
if $GENERATE_GENESIS; then
    ...
    TRUSTED_GENESIS_URL="https://raw.githubusercontent.com/0xPolygon/heimdall-v2/main/genesis.json"
    if [[ "$GENESIS_FILE_URL" != "$TRUSTED_GENESIS_URL" ]]; then
        handle_error $STEP "GENESIS_FILE_URL is not trusted. Only the official source is allowed."
    fi
    echo "[INFO] Downloading genesis file from default source: $GENESIS_FILE_URL"
    if ! curl -fsSL "$GENESIS_FILE_URL" -o "$GENESIS_FILE"; then
        handle_error $STEP "Failed to download genesis file from $GENESIS_FILE_URL"
    fi
    echo "[INFO] Genesis file downloaded to $GENESIS_FILE"
fi


# Step 5: Generate checksum of the genesis export
STEP=5
...
# Step 7: backup heimdall-v1
STEP=7
print_step $STEP "Backing up $HEIMDALL_HOME in $BACKUP_DIR"
ROLLBACK_ACTIONS["$STEP"]="rm -rf \"$BACKUP_DIR\""
# execute command to create the target directory
sudo mkdir -p "$BACKUP_DIR" || handle_error $STEP "Failed to create backup directory $BACKUP_DIR"
# execute command to copy the content
sudo cp -a "$HEIMDALL_HOME/." "$BACKUP_DIR" || handle_error $STEP "Failed to backup the directory $HEIMDALL_HOME in $BACKUP_DIR"
echo "[INFO] Backup completed successfully."


# Step 8: remove original heimdall home
STEP=8
print_step $STEP "Deleting $HEIMDALL_HOME"
ROLLBACK_ACTIONS["$STEP"]="mkdir -p \"$HEIMDALL_HOME\" && cp -a \"$BACKUP_DIR/.\" \"$HEIMDALL_HOME\" || rm -rf \"$HEIMDALL_HOME\""
# execute command to remove original heimdall v1 folder
sudo rm -rf "$HEIMDALL_HOME" || handle_error $STEP "Failed to remove original Heimdall v1 directory $HEIMDALL_HOME"
...
if ! heimdalld migrate "$BACKUP_DIR/$DUMP_V1_GENESIS_FILE_NAME" --chain-id="$CHAIN_ID" --genesis-time="$GENESIS_TIME" --initial-height="$INITIAL_HEIGHT" --verify-hash="$VERIFY_HASH"; then
...
# Init Heimdall v2
if ! heimdalld init "$MONIKER_NODE_NAME" --home="$tmpDirV2Home" &> /dev/null; then
...
echo -e "   👉 TODO: This will be clarified in a future version of this guide.\n"
# ----------------------------------------
...
STEP=30
print_step $STEP "Starting Heimdall v2 service"
if ! sudo systemctl daemon-reload; then
    handle_error $STEP "Failed to run 'sudo systemctl daemon-reload'"
fi
if ! sudo systemctl start heimdalld; then
    handle_error $STEP "Failed to start Heimdall v2 service"
fi
if [ -d "$HEIMDALL_HOME.bak" ]; then
    ROLLBACK_ACTIONS["$STEP"]="sudo systemctl stop heimdalld && mv \"$HEIMDALL_HOME.bak\" \"$HEIMDALL_HOME\""
else
    ROLLBACK_ACTIONS["$STEP"]="sudo systemctl stop heimdalld"
fi
# Wait for the process to come up
for i in {1..5}; do
    HEIMDALL_PID=$(pgrep -f heimdalld | head -n 1)
    if [[ -n "$HEIMDALL_PID" ]]; then
        NEW_HEIMDALL_USER=$(ps -o user= -p "$HEIMDALL_PID" 2>/dev/null)
        break
    fi
    sleep 2
done
if [[ -z "$NEW_HEIMDALL_USER" ]]; then
    handle_error $STEP "Heimdall did not start properly—no process found."
fi
if [[ "$NEW_HEIMDALL_USER" != "$HEIMDALL_SERVICE_USER" ]]; then
    echo "[WARNING] Heimdall is running as $NEW_HEIMDALL_USER instead of $HEIMDALL_SERVICE_USER! Run 'sudo systemctl cat heimdalld' to inspect the service configuration."
fi
echo "[INFO] Heimdall successfully started as user: $NEW_HEIMDALL_USER"
if systemctl list-units --type=service | grep -q "^telemetry.service"; then
    if ! sudo systemctl restart telemetry; then
        handle_error $STEP "Failed to restart telemetry service."
    fi
else
    echo "[INFO] Telemetry service not found, skipping restart."
fi
END_TIME=$(date +%s)
...
echo -e "To verify everything is running correctly, check the logs using:"
